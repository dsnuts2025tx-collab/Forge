...
Every customer-facing production release follows one synchronized machine path:

**USER INTENT → G TRANSLATION → MASTERMIND UNDERSTANDING/PLAN → PHANTOM AUTHORIZATION → FORGE IMPLEMENTATION → TEST → INDEPENDENT VERIFICATION → PHANTOM ACCEPT/REJECT → DEPLOY → LIVE HEALTH/SMOKE PROOF → OBSERVE → RECONCILE → LEARN → EVOLVE**
